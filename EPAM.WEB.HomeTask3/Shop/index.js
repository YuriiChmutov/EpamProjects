...
for (let i = 0; i < arrayOfTelephones.length; i++){
  if (Object.keys(arrayOfTelephones[i].price).length == 2){
    arrayOld.push(arrayOfTelephones[i].price.oldUan);
  }
  else{
    arrayOld.push("hello");
  }
...
for (let i = 0; i < arrayOld.length; i++) {
  if (arrayOld[i] != "hello"){
    arrayOld[i] = Number.parseInt(arrayOld[i].replace(/[A-Za-zА-Яа-яЁё\s]/g, ''));
    sortedOldArray.push(Number.parseInt(arrayOld[i]));
  }
}

sortedOldArray.sort((a,b) => b - a);

let newOldArray = [];

function createSortedArrayOfOldPrices(telephones, indexes, newold, sortedsold) {
  let s = 0;
  let j = 0;
  for (let i = 0; i < telephones.length; i++) {
    if (i === indexes[j]) {
      newold.push(sortedsold[s]);
      s++;
      j++;
    }
    else {
...
createSortedArrayOfOldPrices(arrayOfTelephones, arrayOfIndexOld, newOldArray, sortedOldArray);


  arrayOfTelephones.sort(function (a, b) {
   if (Object.keys(a.price).length == 2) {
    a.price = a.price.newUan;
    } 
    if ( Object.keys(b.price).length == 2) { 
      b.price = b.price.newUan;
    } 
    if (Object.keys(a.price).length != 2 || Object.keys(b.price).length != 2){
  let priceA = a.price.replace(/[A-Za-zА-Яа-яЁё\s]/g, '');
  let priceB = b.price.replace(/[A-Za-zА-Яа-яЁё\s]/g, '');
  priceA = Number(priceA);
  priceB = Number(priceB);
  if (priceA < priceB) //сортируем строки по возрастанию 
  return -1
 if (priceA > priceB)
   return 1
...
function flexim() {
  ...
  bo.style.backgroundColor = "#de2b24";
  bo.style.display = "flex";
  bo.style.flexDirection = "row";
  bo.style.justifyContent = "center";
  bo.style.flexWrap = "wrap";
}

flexim();

let q = arrayOfTelephones.length -1;

function makingStyleandPrint(array, newArray, oldArray){
for (let i = 0; i < array.length; i++) {
document.body.insertAdjacentHTML('afterbegin', '<div id = "mobileBlock"> </div>');
let mobile = document.getElementById("mobileBlock");
mobile.style.height = "150px";
mobile.style.width = "20%"
mobile.style.backgroundColor = "#1287bf";
mobile.style.margin = "auto";
mobile.style.marginTop = "50px";
mobile.style.borderRadius = "5px";
mobile.style.paddingTop = "15%";
mobile.style.fontSize = "15px";
mobile.style.fontFamily = "Arial";
mobile.style.color = "White";
mobile.style.paddingLeft = "10px";
mobile.style.paddingBottom = "2%";
mobile.style.borderStyle = "solid";
mobile.style.borderWidth = "2px";
mobile.style.borderColor = "black";

  for (let key in array[i])  
    mobile.insertAdjacentHTML('afterbegin', array[i][key] + '<br><br>');
  

  if (oldArray[q] != "hello") 
    mobile.insertAdjacentHTML('beforeend', '<strike>' + newArray[q] + '</strike>');

  q--;
  }
}

makingStyleandPrint(arrayOfTelephones, newOldArray, arrayOld);
